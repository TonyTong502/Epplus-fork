...
namespace OfficeOpenXml.Drawing
{
    ...
    public class ExcelDrawing : XmlHelper, IDisposable 
    {
        ...
        public void SetPosition(int Row, int RowOffsetPixels, int Column, int ColumnOffsetPixels)
        {
            ...
            {
                throw new ArgumentException("Minimum negative offset is -60.", "RowOffsetPixels"); // nameof(RowOffsetPixels) the nameof only can use in 4.6 or above.
            }
            if (ColumnOffsetPixels < -60)
            {
                throw new ArgumentException("Minimum negative offset is -60.", "ColumnOffsetPixels");// nameof(RowOffsetPixels) the nameof only can use in 4.6 or above.
            }
            int width = GetPixelWidth();
            int height = GetPixelHeight();
